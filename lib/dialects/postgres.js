...
class PSQLDialect {
  ...
  constructor(Sequelize, connection, opts) {
    ...
    this.typeMap = {
      "USER-DEFINED": Sequelize.GEOMETRY,
    ...
    this.primaryColumnsQuery = `SELECT table_name, column_name
    FROM information_schema.KEY_COLUMN_USAGE where "constraint_schema" = 'public' AND "constraint_name" LIKE '%pkey'`;
  ...
  loadModels(modelDefs, autoload) {
    ...
    return co(function*() {
      ...
      if (autoload) {
        ...
        yieldable.primaryColumns = _this.connection.query(
          _this.primaryColumnsQuery,
          {
            replacements: [_this.config.schema],
            type: _this.connection.QueryTypes.SELECT,
          }
        ...
        yieldable.associations = _this.connection.query(
          _this.relationsQuery,
          {
            replacements: [_this.config.schema],
            type: _this.connection.QueryTypes.SELECT,
          }
      ...
      schemaData.primaryColumns = schemaData.primaryColumns.reduce(
        (p, v) => {
          if (!p.hasOwnProperty(v.table_name)) {
            p[v.table_name] = [];
          }
          if (v.table_name == "Rules") {
            console.log(v);
          }
          if (p[v.table_name].indexOf(v.column_name) === -1) {
            p[v.table_name].push(v.column_name);
          }
          return p;
        },
        {}
      ...
      _this.models = _this.getModels(
        schemaData.tables,
        modelDefs,
        schemaData.primaryColumns
      );
      ...
      schemaData.associations.forEach(association => {
        ...
        // имя ассоциируемой таблицы исключая последнюю букву Users -> User
        const lastLetterDeleted = referencedModelName.substring(0, referencedModelName.length - 1);
        // выражение ищет имя ассоциируемой таблицы внутри начала имени основной таблицы UserGroupUsers <- UserGroup
        const firstRegex = new RegExp(`^${lastLetterDeleted}`);
        const firstMatch = modelName.match(firstRegex);
        // выражение ищет имя ассоциируемой таблицы внутри конца имени основной таблицы UserGroupUsers <- Users
        const secondRegex = new RegExp(`${referencedModelName}$`);
        const secondMatch = modelName.match(secondRegex);
        // имя основой таблицы к которой ассоциируется вторая таблица
        let mainModel;
        let manyToMany = false;

        if (firstMatch) {
          // берем имя основной таблицы из начала имени текущей модели 
          mainModel = modelName.substring(lastLetterDeleted.length, modelName.length);
          // проверка на возможность совпадения имени ассоциируемой таблицы с именем основной таблицы
          manyToMany = Object.keys(_this.models).filter(item => mainModel === item).length > 0 ? true : false;
        }

        if (secondMatch) {
          // берем имя основной таблицы из конца имени текущей модели
          mainModel = modelName.substring(0, modelName.length - referencedModelName.length) + 's';
          // не проверяем, считаем что связь всегда многие-ко-многим
          manyToMany = true;
        }
        // ассоциируем таблицы со связями многие-ко-многим
        if (manyToMany) {
          _this.models[mainModel].belongsToMany(
            _this.models[referencedModelName],
            {
              through: _this.models[modelName],
            }
          );
        } else {
          // ассоциируем все остальные таблицы один-к-одному, один-ко-многим
  ...
  getModels(tables, modelDefs, primaryColumns) {
    debug("getModels");
    ...
    const isColumnPK = res =>
      primaryColumns.hasOwnProperty(res.name) &&
      primaryColumns[res.name].indexOf(res.column_name) !== -1;
      ...
      if (name !== "pg_stat_statements") {
        results.map(res => {
          attribs[res.column_name] = {
            ...
            primaryKey: isColumnPK(res),
            autoIncrement:
              res.column_key && res.column_key.indexOf("nextval") !== -1,
          };
        });
      }
